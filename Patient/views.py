...
class VillageWiseGenderReport(APIView):
          def get(self, request):
                    ...
                    data = Patientopdform.objects.filter(year=year, month=month, village=village).values('villageName',
                                                                                                         'gender')
                    ...
                    # Return the Excel file as a FileResponse
                    response = FileResponse(output, as_attachment=True,
                                            filename=f'VillageWiseGenderReport_{month}.xlsx')
                    return response
...
class VillageWiseAgeGroupReport(APIView):
          def get(self, request):
                    ...
                    if df.empty:
                              return Response(
                                        {"error": f"No data available for the month: {month} in village: {village}"},
                                        status=400)
                    ...
                    # Return the Excel file as a FileResponse
                    response = FileResponse(output, as_attachment=True,
                                            filename=f'VillageWiseAgeGroupReport_{month}.xlsx')
                    return response
...
class MonthlySummaryReport(APIView):
          def get(self, request):
                    ...
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                              ...
                              worksheet.cell(row=2, column=len(pivot_table.columns) + 1,
                                             value="Percentage").font = font_style
                              grand_total_cell = worksheet.cell(row=total_row,
                                                                column=len(pivot_table.columns)).coordinate

                              for row_idx in range(3, total_row):
                                        total_value_cell = worksheet.cell(row=row_idx,
                                                                          column=len(pivot_table.columns)).coordinate
                                        percentage_formula = f"=ROUND(({total_value_cell}/{grand_total_cell})*100, 0)"
                                        cell = worksheet.cell(row=row_idx, column=len(pivot_table.columns) + 1,
                                                              value=percentage_formula)
                                        cell.border = thin_border
                    ...
                    # Return the Excel file as a FileResponse
                    response = FileResponse(output, as_attachment=True,
                                            filename=f'Monthly-Summary-Report_{month}-{year}.xlsx')
                    return response
